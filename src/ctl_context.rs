...
impl Builder {
    ...
    pub fn encode_and_sign(&self) -> io::Result<Vec<u8>> {
        unsafe {
            let encoding = Cryptography::X509_ASN_ENCODING.0 | Cryptography::PKCS_7_ASN_ENCODING.0;

            let mut usages = self
                .usages
                .iter()
                .map(|u| PSTR(u.as_ptr() as _))
                .collect::<Vec<_>>();
            ...
            let mut ctl_info = Cryptography::CTL_INFO::default();
            ...
            ctl_info.SubjectAlgorithm.pszObjId = PSTR(szOID_OIWSEC_sha1.as_ptr() as _);
            ctl_info.cCTLEntry = entries.len() as u32;
            ctl_info.rgCTLEntry = entries.as_mut_ptr();

            let mut sign_info = Cryptography::CMSG_SIGNED_ENCODE_INFO::default();
