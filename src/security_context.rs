...
impl Drop for SecurityContext {
    fn drop(&mut self) {
        unsafe {
            let _ = Identity::DeleteSecurityContext(&self.0);
        }
    }
}
...
impl SecurityContext {
    pub fn initialize(
        ...
        unsafe {
            ...
            match Identity::InitializeSecurityContextW(
                ...
                Ok(_) => Err(io::Error::from_raw_os_error(0)),
    ...
    unsafe fn attribute<T>(&self, attr: Identity::SECPKG_ATTR) -> io::Result<T> {
        ...
        let status = Identity::QueryContextAttributesW(
    ...
    pub fn application_protocol(&self) -> io::Result<Identity::SecPkgContext_ApplicationProtocol> {
        unsafe {
            self.attribute(Identity::SECPKG_ATTR(
