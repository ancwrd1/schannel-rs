...
use windows::core::{HRESULT, PCSTR, PSTR, PWSTR};
        ...
        unsafe {
            ...
            if len > 0 {
                ...
                let ok = Cryptography::CryptEncodeObjectEx(
                    Cryptography::X509_ASN_ENCODING,
                    PCSTR(
                        Cryptography::CERT_INFO_SUBJECT_PUBLIC_KEY_INFO_FLAG as *const u32
                            as *const _,
                    ),
                    &(*(*self.0).pCertInfo).SubjectPublicKeyInfo
                        as *const Cryptography::CERT_PUBLIC_KEY_INFO as _,
    ...
    pub fn valid_uses(&self) -> io::Result<ValidUses> {
        unsafe {
            ...
            if use_cnt == 0 {
                ...
                match last_error.raw_os_error().map(HRESULT) {
            ...
            for i in 0..use_cnt {
                ...
                oids.push(
                    CStr::from_ptr((*(oid_ptr.offset(i as isize))).0 as *const _)
                        .to_string_lossy()
                        .into_owned(),
                );
            }
...
impl<'a> AcquirePrivateKeyOptions<'a> {
    ...
    pub fn compare_key(&mut self, compare_key: bool) -> &mut AcquirePrivateKeyOptions<'a> {
        self.flag(Cryptography::CRYPT_ACQUIRE_COMPARE_KEY_FLAG.0, compare_key)
    }
    ...
    pub fn silent(&mut self, silent: bool) -> &mut AcquirePrivateKeyOptions<'a> {
        self.flag(Cryptography::CRYPT_ACQUIRE_SILENT_FLAG.0, silent)
    }
    ...
    pub fn acquire(&self) -> io::Result<PrivateKey> {
        unsafe {
            let flags = Cryptography::CRYPT_ACQUIRE_FLAGS(
                self.flags | Cryptography::CRYPT_ACQUIRE_ALLOW_NCRYPT_KEY_FLAG,
            );
            ...
            assert!(free.as_bool());
            if spec.0 & Cryptography::CERT_NCRYPT_KEY_SPEC.0 != 0 {
                Ok(PrivateKey::NcryptKey(NcryptKey::from_inner(
                    Cryptography::NCRYPT_KEY_HANDLE(handle.0),
                )))
            } else {
...
impl<'a> SetKeyProvInfo<'a> {
    ...
    pub fn keep_open(&mut self, keep_open: bool) -> &mut SetKeyProvInfo<'a> {
        self.flag(Cryptography::CERT_SET_KEY_PROV_HANDLE_PROP_ID.0, keep_open)
    }
    ...
    pub fn set(&mut self) -> io::Result<()> {
        unsafe {
            ...
            let info = Cryptography::CRYPT_KEY_PROV_INFO {
                pwszContainerName: PWSTR(container as *mut _),
                pwszProvName: PWSTR(provider as *mut _),
                dwProvType: self.type_,
                dwFlags: Cryptography::CRYPT_KEY_FLAGS(self.flags),
                cProvParam: 0,
