...
use windows::core::{PCWSTR, PSTR, PWSTR};
use windows::Win32::Foundation;
use windows::Win32::NetworkManagement::NetManagement;
use windows::Win32::Security::Cryptography;
use windows::Win32::System::{SystemInformation, Time};
...
fn invalid_algorithms() {
    ...
    assert_eq!(
        ...
        Foundation::SEC_E_ALGORITHM_MISMATCH.0 as i32
    );
}
...
fn invalid_protocol() {
    ...
    assert_eq!(
        ...
        Foundation::SEC_E_UNSUPPORTED_FUNCTION.0 as i32
    );
}
...
fn expired_cert() {
    ...
    assert_eq!(
        err.raw_os_error().unwrap(),
        Foundation::CERT_E_EXPIRED.0 as i32
    );
}
...
fn self_signed_cert() {
    ...
    assert_eq!(
        err.raw_os_error().unwrap(),
        Foundation::CERT_E_UNTRUSTEDROOT.0 as i32
    );
}
...
fn wrong_host_cert() {
    ...
    assert_eq!(
        err.raw_os_error().unwrap(),
        Foundation::CERT_E_CN_NO_MATCH.0 as i32
    );
}
    ...
    let err = tls_stream::Builder::new()
        ...
        .verify_callback(|validation_result| {
            ...
            Err(io::Error::from_raw_os_error(
                Foundation::CERT_E_UNTRUSTEDROOT.0,
            ))
        })
        .connect(creds, stream)
        .err()
    ...
    let err = tls_stream::Builder::new()
        ...
        .verify_callback(|validation_result| {
            let expected_finger = vec![
                0x99, 0xc1, 0xda, 0xf0, 0x7c, 0x8d, 0x69, 0xa8, 0xa0, 0x65, 0x49, 0x2d, 0xca, 0xae,
                0x43, 0xc4, 0x3f, 0xf1, 0x34, 0x97,
            ];
            assert_eq!(
                validation_result
                    .failed_certificate()
...
fn install_certificate() -> io::Result<CertContext> {
    unsafe {
        ...
        let res = Cryptography::CryptAcquireContextW(
            ...
            Cryptography::CRYPT_MACHINE_KEYSET.0,
        );
        if !res.as_bool() {
            ...
            let res = Cryptography::CryptAcquireContextW(
                ...
                PCWSTR::default(),
                Cryptography::PROV_RSA_FULL,
                Cryptography::CRYPT_NEWKEYSET | Cryptography::CRYPT_MACHINE_KEYSET.0,
            );
            if !res.as_bool() {
        ...
        let res = Cryptography::CryptGenKey(
            provider,
            Cryptography::AT_SIGNATURE.0,
            Cryptography::CRYPT_KEY_FLAGS(0x0800 << 16 | Cryptography::CRYPT_EXPORTABLE.0),
            &mut hkey,
        );
        if !res.as_bool() {
            return Err(Error::last_os_error());
        ...
        let name = "CN=localhost,O=schannel-rs,OU=schannel-rs,G=schannel_rs"
            ...
            .chain(Some(0))
            .collect::<Vec<_>>();
        let mut cname_buffer: [u16; NetManagement::UNLEN as usize + 1] = mem::zeroed();
        let mut cname_len = cname_buffer.len() as u32;
        let res = Cryptography::CertStrToNameW(
            Cryptography::X509_ASN_ENCODING.0,
            PCWSTR(name.as_ptr()),
        ...
        let key_provider = Cryptography::CRYPT_KEY_PROV_INFO {
            pwszContainerName: PWSTR(buffer.as_mut_ptr()),
            pwszProvName: PWSTR::default(),
            dwProvType: Cryptography::PROV_RSA_FULL,
            dwFlags: Cryptography::CRYPT_MACHINE_KEYSET,
            cProvParam: 0,
            rgProvParam: ptr::null_mut(),
            dwKeySpec: Cryptography::AT_SIGNATURE.0,
        };
        let sig_algorithm = Cryptography::CRYPT_ALGORITHM_IDENTIFIER {
            pszObjId: PSTR(szOID_RSA_SHA256RSA.as_ptr() as *mut _),
            Parameters: mem::zeroed(),
        };
        let mut expiration_date = Foundation::SYSTEMTIME::default();
        SystemInformation::GetSystemTime(&mut expiration_date);
        let mut file_time = Foundation::FILETIME::default();
