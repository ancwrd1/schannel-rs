...
impl CertValidationResult {
    ...
    pub fn result(&self) -> io::Result<()> {
        if self.res as u32 != Foundation::ERROR_SUCCESS.0 {
    ...
    S: Read + Write,
{
        ...
        unsafe {
            ...
            let status = if self.server {
                ...
                Identity::AcceptSecurityContext(
                    ...
                    ptr,
                    &inbuf_desc,
                    Identity::ACCEPT_SECURITY_CONTEXT_CONTEXT_REQ(ACCEPT_REQUESTS),
                    0,
                    self.context.get_mut(),
                    &mut outbuf_desc,
                    &mut attributes,
            ...
            for buf in &outbufs[1..] {
                if !buf.pvBuffer.is_null() {
                    let _ = Identity::FreeContextBuffer(buf.pvBuffer);
                }
            }
            ...
            match status {
                ...
                Err(e) if e.code() == Foundation::SEC_I_CONTINUE_NEEDED => {
                ...
                Err(e) if e.code() == Foundation::SEC_E_INCOMPLETE_MESSAGE => {
    ...
    fn validate(&mut self, require_cert: bool) -> io::Result<bool> {
        ...
        let cert_chain = unsafe {
            let cert_store = match (cert_context.cert_store(), &self.cert_store) {
                ...
                (None, &None) => Cryptography::HCERTSTORE::default(),
            };
            ...
            let mut identifiers = [
                PSTR(szOID_PKIX_KP_SERVER_AUTH.as_ptr() as _),
                PSTR(szOID_SERVER_GATED_CRYPTO.as_ptr() as _),
                PSTR(szOID_SGC_NETSCAPE.as_ptr() as _),
            ];
        ...
        unsafe {
            ...
            if let Some(ref mut store) = self.cert_store {
                if let Some(chain) = cert_chain.final_chain() {
                    ...
                    {
                        para_flags.0 |= Cryptography::CERT_CHAIN_POLICY_ALLOW_UNKNOWN_CA_FLAG.0;
                    }
                }
            }
            ...
            match self.domain {
                Some(ref mut domain) if !self.accept_invalid_hostnames => {
                    extra_para.pwszServerName = PWSTR(domain.as_mut_ptr());
                }
                _ => {}
            }
            ...
            let mut verify_result = if status.dwError != Foundation::ERROR_SUCCESS.0 {
    ...
    fn decrypt(&mut self) -> io::Result<bool> {
        unsafe {
            ...
            match Identity::DecryptMessage(self.context.get_mut(), &bufdesc, 0) {
                Ok(_) => {
                ...
                Err(e) if e.code() == Foundation::SEC_I_CONTEXT_EXPIRED => Ok(true),
                Err(e) if e.code() == Foundation::SEC_I_RENEGOTIATE => {
                    self.state = State::Initializing {
                        needs_flush: false,
                        more_calls: true,
                        shutting_down: false,
        ...
        let sizes = match self.initialize()? {
            ...
            None => {
                return Err(io::Error::from_raw_os_error(
                    Foundation::SEC_E_CONTEXT_EXPIRED.0 as i32,
                ))
            }
        };
