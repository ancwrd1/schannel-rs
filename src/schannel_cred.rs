...
impl Builder {
    ...
    pub fn supported_algorithms(&mut self, supported_algorithms: &[Algorithm]) -> &mut Builder {
        assert!(supported_algorithms.iter().all(|a| {
            match *a {
    ...
    pub fn enabled_protocols(&mut self, enabled_protocols: &[Protocol]) -> &mut Builder {
        assert!(enabled_protocols.iter().all(|a| {
            match *a {
    ...
    pub fn acquire(&self, direction: Direction) -> io::Result<SchannelCred> {
        unsafe {
            let mut handle = Credentials::SecHandle::default();
            let mut cred_data = Identity::SCHANNEL_CRED::default();
            ...
            match Identity::AcquireCredentialsHandleA(
                PCSTR::default(),
                PCSTR(UNISP_NAME.as_ptr() as *const _ as *mut _),
                direction,
                ptr::null_mut(),
                ...
                Ok(_) => Ok(SchannelCred::from_inner(handle)),
...
impl Drop for RawCredHandle {
    fn drop(&mut self) {
        unsafe {
            let _ = Identity::FreeCredentialsHandle(&self.0);
        }
    }
}
